...
export default function ServicesPage() {
  ...
  const removeImage = async (image) => { if (image.pending) { setImages(images.filter((item) => item.id !== image.id)); return } if (!editing) return; setImageBusy(true); setFeedback(''); try { await deleteServiceImage(editing.id, image.id); syncImages(images.filter((item) => item.id !== image.id)) } catch (requestError) { setFeedback(getError(requestError)) } finally { setImageBusy(false) } }
