...
export default function ServicesPage() {
  ...
  const columns = useMemo(() => [{ key: 'name', label: 'Nama layanan', render: (row) => <strong>{row.name}</strong> }, { key: 'price', label: 'Harga', render: (row) => formatCurrency(row.price) }, { key: 'is_active', label: 'Status', render: (row) => <span className={`text-status ${row.is_active ? 'is-active' : 'is-inactive'}`}>{row.is_active ? 'Aktif' : 'Nonaktif'}</span> }, { key: 'images', label: 'Galeri', render: (row) => `${row.images?.length ?? 0} foto` }, { key: 'actions', label: 'Aksi', render: (row) => <div className="table-actions"><button className="table-action" type="button" onClick={() => openEdit(row)}><PencilSimpleIcon size={15} /> Edit</button><button className="table-action table-action-danger" type="button" onClick={() => { openEdit(row); setConfirmOpen(true) }}><TrashIcon size={15} /> Hapus</button></div> }], [])

  return <AdminLayout title="Layanan" description="Atur katalog layanan dan harga yang tampil ke klien." onRefresh={loadServices} isLoading={isLoading} action={<button className="admin-button admin-button-primary" type="button" onClick={openCreate}><PlusIcon size={16} /> Layanan baru</button>}>
    {feedback && !editing && <div className="admin-alert" role="status">{feedback}</div>}
    <AdminDataTable columns={columns} rows={rows} isLoading={isLoading} error={error} onRetry={loadServices} emptyMessage={<span>Belum ada layanan. <button className="table-action" type="button" onClick={openCreate}>Tambah layanan pertama</button></span>} />
