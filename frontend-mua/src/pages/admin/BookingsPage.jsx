...
export default function BookingsPage() {
  ...
  return <AdminLayout title="Booking" description="Kelola pengajuan, jadwal, dan status pekerjaan." onRefresh={loadBookings} isLoading={isLoading} action={<Link className="admin-button admin-button-primary" to="/booking"><PlusIcon size={16} /> Booking publik</Link>}>
    ...
    <AdminDrawer open={Boolean(selected)} title={selected?.client_name ?? 'Booking'} onClose={() => setSelected(null)}>
      {drawerLoading ? <div className="admin-state">Memuat detail...</div> : selected && <BookingDetail booking={selected} staff={staff} role={sessionUser?.role ?? 'staff'} feedback={feedback} onSave={saveBooking} onDelete={() => setConfirmOpen(true)} onSendPaymentLink={sendPaymentLink} />}
    </AdminDrawer>
...
function BookingDetail({ booking, staff, role, feedback, onSave, onDelete, onSendPaymentLink }) {
